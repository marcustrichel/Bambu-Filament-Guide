...
create table print_profiles (
  ...
  -- Expanded to mirror the full Bambu Studio Quality tab (Layer height, Line
  -- width, Seam, Precision, Ironing, Wall generator, Advanced). Elephant foot
  -- compensation moved here from `others`, matching current Bambu Studio.
  ...
  -- Expanded to mirror the full Bambu Studio Strength tab (Walls, Top/bottom
  -- shells, Sparse infill, Advanced).
  ...
  -- Expanded to mirror the full Bambu Studio Speed tab (Initial layer speed,
  -- Other layers speed, Travel speed, Acceleration). Only the fields that
  -- already varied by printer_model above (outer_wall, inner_wall,
  -- sparse_infill, solid_infill, top_surface, first_layer, travel,
  -- acceleration) still do — the rest are flat defaults for every model.
  ...
  -- Expanded to mirror the full Bambu Studio Support tab (Support, Raft,
  -- Filament for Supports, Advanced). Support type values now include the
  -- "(auto)" suffix Bambu Studio itself displays.
  ...
  -- Expanded to mirror the full Bambu Studio Others tab (Bed adhesion, Prime
  -- tower, Purge options, Special mode, Advanced, G-code output,
  -- Post-processing scripts, Notes). elephant_foot_compensation moved to
  -- `quality` above, matching current Bambu Studio; notes/post_processing_scripts
  -- live here as plain JSONB fields since, unlike the filament editor, this tab
  -- is where Bambu Studio actually places them (no separate top-level tab).
