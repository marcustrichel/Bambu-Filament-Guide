...
*   **Deploy:** `supabase functions deploy update-user-email` (requires `supabase login` or a `SUPABASE_ACCESS_TOKEN`, neither of which this repo or its automation holds). `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` are injected automatically by the platform — no secrets to configure by hand.
